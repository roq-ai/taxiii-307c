...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createCompany() {
    ...
    const body = { ...req.body };
    if (body?.admin_staff?.length > 0) {
      const create_admin_staff = body.admin_staff;
      body.admin_staff = {
        create: create_admin_staff,
      };
    } else {
      delete body.admin_staff;
    }
    if (body?.cab?.length > 0) {
      const create_cab = body.cab;
      body.cab = {
        create: create_cab,
      };
    } else {
