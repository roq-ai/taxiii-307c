...
import { getAdminStaffs } from 'apiSdk/admin-staffs';
import { AdminStaffInterface } from 'interfaces/admin-staff';

import { CabListPage as ShowCaseList } from 'pages/cabs';

function DashboardPage() {
  const { session } = useSession();

  const cabStat = useSWR<PaginatedInterface<CabInterface>>(
    () => `/cabs`,
    () => getCabs({ limit: 1, offset: 0 }),
  );
  ...
  const adminStaffStat = useSWR<PaginatedInterface<AdminStaffInterface>>(
    () => `/admin-staffs`,
    () => getAdminStaffs({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Cabs',
      result: cabStat?.data?.totalCount,
      isLoading: cabStat.isLoading,
    ...
    {
      title: 'Admin Staffs',
      result: adminStaffStat?.data?.totalCount,
      isLoading: adminStaffStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`cabs`} passHref legacyBehavior>
                  <Button
