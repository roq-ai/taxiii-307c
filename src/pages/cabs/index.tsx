...
import { getCabs, deleteCabById } from 'apiSdk/cabs';
import { CabInterface } from 'interfaces/cab';

type ColumnType = ColumnDef<CabInterface, unknown>;

interface CabListPageProps {
...
export function CabListPage(props: CabListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getCabs({
        ...
        order: params.order,
        searchTermKeys: ['model.contains', 'license_plate.contains', 'status.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<CabInterface>>(
    () => `/cabs?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: CabInterface) => {
    if (hasAccess('cab', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/cabs/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/cabs/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('cab', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/cabs/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('cab', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="cab" />
        </Flex>

        {hasAccess('cab', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/cabs/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
